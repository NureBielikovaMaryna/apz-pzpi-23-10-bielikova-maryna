...
// !!! ПРОВЕРЬ ID ТУННЕЛЯ ПЕРЕД ЗАПУСКОМ !!!
const BASE_URL = "https://7rps4v1s-5152.euw.devtunnels.ms/api";
const AUTH_API = `${BASE_URL}/Auth`;
const TRAINS_API = `${BASE_URL}/Trains`;
        ...
        {(view === 'schedule' || view === 'admin' || view === 'history') && (
          <div style={{ background: '#fff', padding: '24px', borderRadius: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
               <Space>
                 <Button type={view === 'schedule' ? 'primary' : 'default'} onClick={() => setView('schedule')}>{t('train_schedule')}</Button>
                 {role === 'Client' && <Button icon={<HistoryOutlined />} onClick={async () => {
                    const res = await axios.get(`${AUTH_API}/history/${userId}`, { headers });
                    setHistory(res.data); setView('history');
                 }}>{t('my_history')}</Button>}
                 {role === 'Admin' && <Button icon={<SettingOutlined />} onClick={() => setView('admin')}>{t('admin_panel')}</Button>}
               </Space>
            ...
            {view === 'admin' ? (
              ...
              <Table dataSource={history} columns={[{ title: t('train_number'), dataIndex: 'trainNumber' }, { title: t('date'), dataIndex: 'viewedAt' }]} rowKey="viewedAt" />
            ) : (
              <Table dataSource={trains} columns={columns} rowKey="id" />
            )}
          </div>
        )}
